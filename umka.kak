provide-module -override umka %§
	...
	add-highlighter shared/umka/code/ regex "\b([A-Za-z_]\w*)\b\h*(\*\h*)?(?=\()" 1:function

	add-highlighter shared/umka/code/ regex \
		'\b(break|case|const|continue|default|else|enum|fn|for|interface|if|in|map|return|struct|switch|type|var|weak)\b' 0:keyword
	...
	add-highlighter shared/umka/code/ regex 'type\h+(?i)([a-z_]\w*)'  1:type
	add-highlighter shared/umka/code/ regex 'const\h+(?i)([a-z_]\w*)' 1:value

	add-highlighter shared/umka/code/ regex '(?i)([a-z_]\w*)(?:\h*,\h*([a-z_]\w*))\h*(?=:=)' 1:variable

	add-highlighter shared/umka/code/ regex '\b[A-Z]\w*\b' 0:type
